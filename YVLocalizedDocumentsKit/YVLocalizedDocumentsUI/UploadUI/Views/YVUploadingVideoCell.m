...
@implementation YVUploadingVideoCell
...
- (void)setupInterface
{
    ...
    [self addConstraints:@[
                           [NSLayoutConstraint constraintWithItem:self.logo attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:0.0 constant:60],//宽度
                           [NSLayoutConstraint constraintWithItem:self.logo attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:0.0 constant:60],//高度
    ...
    self.name.font = [UIFont systemFontOfSize:16.f];
    self.name.lineBreakMode = NSLineBreakByTruncatingMiddle;
    self.name.translatesAutoresizingMaskIntoConstraints = NO;
    [self addSubview:self.name];
    [self addConstraints:@[
                           [NSLayoutConstraint constraintWithItem:self.name attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:0.0 constant:30],//高度
                           [NSLayoutConstraint constraintWithItem:self.name attribute:NSLayoutAttributeTop relatedBy:0 toItem:self.logo attribute:NSLayoutAttributeTop multiplier:1.0 constant:0],//上
    ...
    [self addConstraints:@[
                           ...
                           [NSLayoutConstraint constraintWithItem:self.uploadSize attribute:NSLayoutAttributeTop relatedBy:0 toItem:self.name attribute:NSLayoutAttributeBottom multiplier:1.0 constant:7.f],//上
...
{
    self.logo.image = contentModel.cover;
    ...
    else
    {
        ...
        self.uploadSize.textColor = [UIColor grayColor];
        //        self.uploadVariable.text = [NSString stringWithFormat:@"+%@/s", [[MDCacheManager shareManager] sizeExplain:contentModel.uploadVariable]];
    }
}
