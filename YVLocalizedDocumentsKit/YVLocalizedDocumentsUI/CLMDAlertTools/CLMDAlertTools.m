...
//  iOS_SuperBlocks
//
//  Created by ModouTechnology on 16/10/12.
...
//类别，扩充属性
@interface UIAlertView (CLMDAlertTools)
@property (nonatomic, copy) CallBackBlock clickBlock;
@end

//runtime内联机制，实现set/get方法
@implementation UIAlertView (CLMDAlertTools)
...
//类别，扩充属性
@interface UIActionSheet (CLMDAlertTools)
@property (nonatomic, copy) CallBackBlock clickBlock;
@end

//runtime内联机制，实现set/get方法
@implementation UIActionSheet (CLMDAlertTools)
- (void)setClickBlock:(CallBackBlock)block {
    objc_setAssociatedObject(self, @selector(clickBlock), block, OBJC_ASSOCIATION_COPY);
}
- (CallBackBlock)clickBlock {
...
@interface CLMDAlertTools ()<UIAlertViewDelegate, UIActionSheetDelegate>

@end

@implementation CLMDAlertTools
...
+ (void)showTipAlertViewWith:(UIViewController *)viewController
                 ...
                 buttonStyle:(CLMDAlertActionStyle )btnStyle {
    if (iOS_Version >= 8.0)
    {
        ...
        if (btnTitle) {
            ...
            switch (btnStyle) {
                case CLMDAlertActionStyleDefault:
...
+ (BOOL)converContentIsNull:(id)obj{
    if (!obj) return YES;
    //字符串
    if ([obj isKindOfClass:[NSString class]]) {
        if ([obj isEqualToString:@"null"] || [obj isEqual:[NSNull null]]|| [obj isEqualToString:@""]) {
            return YES;
        }
    }
    //数组
...
#pragma mark << ===单个设备启用语音提示 ====== >>
+(void)speakVoice:(NSString*)string{
    AVSpeechUtterance*utterance = [AVSpeechUtterance speechUtteranceWithString:string]; utterance.pitchMultiplier=1.1; //中式发音
    AVSpeechSynthesisVoice*voice = [AVSpeechSynthesisVoice voiceWithLanguage:@"zh-TW"]; utterance.voice= voice;
